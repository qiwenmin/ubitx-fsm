...
void UiTask::init() {
  ...
  _last_fbutton_state = fbuttonTask.getButtonState();
...
void UiTask::in_state(int8_t state) {
  if (_last_fbutton_state != fbuttonTask.getButtonState()) {
    if (_last_fbutton_state == LOW) {
      gotoState(_current_state == MENU_MAIN ? MENU_NONE : MENU_MAIN);
    }
    _last_fbutton_state = fbuttonTask.getButtonState();
  }
  ...
  switch (state) {
  case MENU_MAIN:
    if (encoderTask.get_value() != 0) {
      if (encoderTask.get_value() > 1) {
        _menu_idx ++;
        if (_menu_idx > 8) _menu_idx = 8;
        else update_display(this);
      } else if (encoderTask.get_value() < -1) {
        _menu_idx --;
...
void UiTask::update_rig_display() {
  ...
  // Lock?
  displayTask.print(8, 0, rig.getDialLock() == ON ? "LCK" : "   ");
